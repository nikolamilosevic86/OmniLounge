...
`configure_character_appearance` (`room:character:appearance`) reuses the same option set and validation as player avatars (`server/game/avatar.py`), so AI characters render through the identical SVG avatar renderer as players — skin color, body type, hair, beard, glasses, clothes, and accessory. The builder panel's "Appearance" section live-updates a preview as each dropdown changes, independent of the "Save Appearance" click.
