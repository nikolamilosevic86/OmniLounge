...
| Content list edit action (§8.6) | Icon button, sibling to the existing `delete` icon | Reuses `.builder-object-row-actions`; no new list component |
| Puzzle template picker (§8.6) | Elevated cards, same shape as catalog cards | Replaces `puzzle-template-select`; reuses the §7.1/§9 card component |
| Link field live preview (§8.6) | Inline thumbnail + status caption | New, but tiny — reuses `.builder-hint`/`.builder-error` typography |
...
### 8.6 Redesigning the content itself: books, videos, tracks, dialogue, knowledge & puzzles
...
- [ ] Add an `edit` icon beside `delete` on Book/Video/Track/Puzzle list rows, wired to the same populate-form/"Save Changes"/Cancel flow `enterKnowledgeDocumentEditMode` already implements for Knowledge Store documents; validate before removing so a failed save can't lose the item (§8.6).
- [ ] Live YouTube link preview + inline validation caption on `video-youtube-input`/`track-youtube-input`, reusing `extractYoutubeVideoId()` on `input` instead of only on submit (§8.6).
- [ ] Replace `puzzle-template-select` with an elevated-card picker showing each `PUZZLE_TEMPLATES` entry's label + description (§8.6).
