...
| `room:escape:leaderboard:global` | client → server | *(Phase 3, §16 Q4)* Fetch the fastest escapes across **all** rooms; each entry is annotated with its source `roomId`/`roomName`, since times from different rooms are not otherwise comparable |
...
| `room:puzzle:templates` | client → server | *(Phase 3, §16 Q5)* Fetch the pre-built puzzle template catalog for the builder's authoring panel. Not permission-gated — static content revealing no room state — and a template never carries an `answer` |
| `room:puzzle:analytics` | client → server | *(Phase 3)* Room-host gated difficulty stats, aggregated **per puzzle across all visitors, never per named visitor**. Pass a `puzzleId` for one puzzle or omit it for all. Includes `commonWrongGuesses`; a *correct* guess is never recorded, so this can't leak the answer |
...
- [x] `StoryEngine.talk`/`advance` reads `knowledge_check`/`completion_flag` and consults `PuzzleEngine.is_solved` before allowing gated progression (§6.4). **Implementation:** `StoryEngine.talk` gained an optional `is_solved: Callable[[str, str], bool] | None = None` callback parameter (mirroring the existing `ask_generative`'s `caller` callback-injection pattern, since `StoryEngine` has no direct dependency on `PuzzleEngine`). When a choice is made (`choice_index` given) and the *current* node's `knowledgeCheck` is set, the callback is consulted with `(knowledge_check, user_id)`; if it returns `False` the choice is blocked — progress does not advance, and the response is flagged `knowledgeCheckPassed: False` — instead of moving to the configured next node. Omitting the callback never blocks progression (backward compatible with every pre-existing caller). `RoomBuilderState.talk_to_character` (the sole path through which a client-supplied `choice_index` flows) now passes `is_solved=self._puzzles.is_solved`. `completion_flag` needed no additional behavior beyond what it already had (a client-facing marker on terminal nodes, already included in every returned node payload) — no new gating behavior was specified for it beyond `knowledge_check`. Client: `dialogue-modal`'s choice handler now shows a brief system message ("You haven't figured that out yet.") when `knowledgeCheckPassed === false`. See `TestKnowledgeCheckGatesProgression` in [tests_python/test_story.py](../tests_python/test_story.py) and the two new cases in `TestAiCharacterIntegration` in [tests_python/test_room_builder.py](../tests_python/test_room_builder.py).
...
- [x] Multi-door / multi-room chained escape sequences using `destinationTile` (§8.3). **Implementation:** new `RoomsRegistry.warp_player_to_tile(player_id, room_id, destination_tile)` performs a direct (non-edge-adjacent) tile transition — unlike `transition_player_tile_if_needed`, the destination need not be a neighbor of the player's current tile — returning the same `{"tile": ..., "position": ...}` shape (arrival position is the room center via the existing `create_position()` default) so callers can treat it the same way as a normal edge crossing; returns `None` defensively if the door's `destinationTile` doesn't exist in the room's tile set. `server/main.py: room_object_interact` now warps the visitor (updates `player["tile"]`/position, then calls the existing `broadcast_room_state`) whenever an `escape_door`'s `attempt_open` succeeds and `destinationTile` is set — including on a repeat open of an already-open door, since walking through a door again is expected. No client changes were needed: `room:state`'s existing per-player `tile` field and the client's existing tile-change handling (`refreshCanvasBuilderObjects`, `tileTransitionDirection` returning `null`/no-op for a non-adjacent jump) already cover this transition kind. See `TestDoorDestinationTileWarp` in [tests_python/test_main_escape_room.py](../tests_python/test_main_escape_room.py) and the new `warp_player_to_tile` cases in [tests_python/test_rooms_registry_tiles.py](../tests_python/test_rooms_registry_tiles.py).
...
- [x] **Bug fix — a removed puzzle left doors permanently unopenable (found during review, not a pre-existing checklist item):** `add_puzzle(unlock_door_id=...)` wires the link by appending the puzzle id to the door's `config["requiredPuzzleIds"]`, but nothing ever unwired it again, and `_attempt_open_door` gates on `all(self._puzzles.is_solved(pid, requester_id) for pid in required_puzzle_ids)` where `PuzzleEngine.is_solved` returns `False` for a puzzle id it has never heard of. So a stale id left behind on a door made that door's AND-gate unsatisfiable **for every visitor, forever** — precisely the "a creator builds an unsolvable room" risk called out in §15, reachable through the builder panel's ordinary Delete-puzzle button. There were two routes in: (1) `RoomBuilderState.remove_puzzle` dropped the puzzle from `PuzzleEngine` without touching any door config; (2) `delete_object`'s cascade used `_puzzle_id_still_referenced`, which only inspected other objects' `config["puzzleId"]` and `config["guardsPuzzleId"]` and so did **not** treat a door's `requiredPuzzleIds` as a reference — deleting an unrelated object that happened to mention the puzzle would garbage-collect a puzzle a door still gated on. Fixed on both sides: `_puzzle_id_still_referenced` now also counts membership in any object's `requiredPuzzleIds`, and a new `_unwire_puzzle_from_doors` helper strips the id from every door whenever a puzzle is genuinely removed, restoring the symmetry with `add_puzzle`'s wiring. See `TestRemovePuzzleUnwiresDoors` in [tests_python/test_room_builder.py](../tests_python/test_room_builder.py).
- [x] **Bug fix — two handlers crashed on a malformed payload (found during review, not a pre-existing checklist item):** `server/main.py: room_object_delete` indexed `data["objectId"]` inside a `try` that caught only `PermissionError`, and `room_media_sync_leave` did the same inside a `try` that caught only `ValueError` (its own sibling `room_media_sync_join` correctly caught `(KeyError, ValueError)`). A client emitting `{}` — or, for the media handler, an unknown `objectId`, since `_require_object` raises `KeyError` — therefore raised out of the handler instead of returning the standard `error` event. Both now catch `KeyError` alongside their existing exception types, matching the convention used by every other handler in the file; `room_media_sync_leave` additionally hoists `object_id` into the `try` so the post-`try` `_broadcast_sync_session` call no longer re-indexes the raw payload. See `TestHandlersSurviveMissingObjectId` in [tests_python/test_main_handlers.py](../tests_python/test_main_handlers.py).
- [x] **Bug fix — whispers were silently dropped while anyone was walking (found during review, not a pre-existing checklist item):** `client/js/main.js: updateRecipientList` reset `recipientDropdown.innerHTML` on every call, and it is called from the `room:state` socket handler — which the server broadcasts from the game loop on *every tick in which any player moved*. The wholesale rebuild discarded the user's chosen recipient many times a second, so `sendMessage`'s `const recipientId = recipientDropdown.value; if (!recipientId) return;` guard bailed out and the whisper vanished with no feedback. Net effect: you could not whisper anybody while anyone else in the room was moving. Fixed by extracting the decision into three pure, unit-tested helpers in [src/chat.js](../src/chat.js) (mirrored verbatim into `client/js/chat.js` per the existing duplication convention) — `whisperRecipientOptions` derives the id/label pairs, `recipientOptionsChanged` skips the DOM work entirely when the roster is unchanged (which is the overwhelmingly common case on a movement tick), and `resolveRecipientSelection` restores the prior selection when a rebuild really is needed and the chosen player is still present, following the `previousValue` idiom already used by `renderEscapeDoorSelect`. See the `whisper recipient list` describes in [tests/chat.test.js](../tests/chat.test.js).
...
- [x] **Bug fix (found during review, not a pre-existing checklist item):** `server/main.py: room_escape_start` called `EscapeSessionEngine.start()` (via `RoomBuilderState.start_escape_session`) with no `try/except`, unlike every sibling escape-room handler — `start()` raises `PermissionError` when escape mode hasn't been `configure`d/enabled for the room yet, so a client (or a malicious client bypassing the UI, which normally hides the Start control until configured) calling `room:escape:start` first would hit an unhandled exception in the handler instead of a clean `error` event. Fixed by wrapping the call in `try/except PermissionError` and emitting `{"message": str(exc)}` via the standard `error` event, matching the pattern used by every other handler in this file. See `test_start_before_escape_mode_is_configured_emits_error_not_a_crash` in [tests_python/test_main_escape_room.py](../tests_python/test_main_escape_room.py).
- [x] **Bug fix — invisible obstacles in non-lobby rooms (reported by user, not a pre-existing checklist item):** `server/game/movement.py`'s module-level `OBSTACLES` list holds the **lobby's** four hardcoded furniture AABBs (`sofa-left`, `sofa-right`, `table`, `dj-deck`), mirroring the branded furniture `client/js/room-renderer.js` draws in `drawFurniture(ctx)` — which it only ever does while `_isLobby` is true (`currentRoomId === 'lobby'`); custom/user-built rooms deliberately render an empty room to be furnished via the room builder. The server, however, iterated that global list unconditionally in all three collision paths (`collides_with_obstacle`, `_find_blocking_obstacle`, and `resolve_collision`'s `embedded_in` computation) with no room or tile scoping, so **every** room — including a brand-new empty one, and every non-origin tile — inherited four invisible, unwalkable boxes at the lobby's furniture coordinates. This directly contradicted §5.1's model that a custom room's collision comes solely from builder-placed objects (`server/main.py: _tile_collision_obstacles` → `extra_obstacles`). Fixed by threading an `include_lobby_obstacles: bool = True` parameter (defaulting to `True`, so no pre-existing caller changes behavior) through `collides_with_obstacle`/`_find_blocking_obstacle`/`resolve_collision`/`move_toward`/`move_by_direction`, factoring the base+extra concatenation into a shared `_all_obstacles` helper, and exporting a `LOBBY_ROOM_ID` constant. `server/main.py: apply_player_movement` now passes `include_lobby_obstacles=(room_id == LOBBY_ROOM_ID)`, and `server/game/room.py: Room._pick_spawn_position` passes `include_lobby_obstacles=(self.id == LOBBY_ROOM_ID)` so spawns in custom rooms no longer dodge phantom furniture either. Builder-placed collision (`extra_obstacles`) and the §5.1 per-visitor `escape_door` skip are unaffected in every room, and the "walk out of an object placed on top of you" escape behavior is preserved. See `TestLobbyObstaclesAreLobbyOnly` in [tests_python/test_movement.py](../tests_python/test_movement.py) and `TestLobbyFurnitureDoesNotBlockOtherRooms` in [tests_python/test_main_movement.py](../tests_python/test_main_movement.py) (which includes regression guards that the lobby still blocks its own visible furniture and that builder objects still block inside custom rooms).



### Phase 3
- [x] Puzzle template library (pre-built riddle/cipher/sequence puzzle types with built-in `match_mode` presets). **Resolves §16 Q5 in favour of shipping templates in Phase 3** (free-text/numeric matching was indeed sufficient to start). Implemented as a new pure module [server/game/puzzle_templates.py](../server/game/puzzle_templates.py) holding a five-entry `PUZZLE_TEMPLATES` catalog (`riddle`, `cipher`, `sequence`, `number_lock`, `keyword_search`), each carrying a `label`, `description`, `promptTemplate`, `answerPlaceholder`, `matchMode` preset, and starter `hints`. `get_template`/`list_templates` deep-copy their records so a caller can never mutate the shared catalog, and `build_puzzle_from_template` returns exactly `PuzzleEngine.add_puzzle`'s kwargs. `RoomBuilderState.add_puzzle` gained a `template_id` parameter that fills in `prompt`/`hints`/`match_mode` while letting any explicitly-passed value override the preset; `RoomBuilderState.list_puzzle_templates` exposes the catalog (deliberately *not* host-gated — it is static content revealing no room state). **A template never supplies the `answer`:** the creator always authors their own secret, so a shipped template can't become a universally-known skeleton key. New `room:puzzle:templates` socket handler, plus a `#puzzle-template-select` picker in the builder's Puzzles panel that prefills the form via the pure `puzzleTemplateFormValues` helper. **Bug fixed in passing:** `room_puzzle_add` defaulted `match_mode` to `"exact"` at the handler layer, which would have silently clobbered every template's `matchMode` preset; the default now lives solely in `RoomBuilderState.add_puzzle`. See `TestTemplateCatalog`/`TestBuildPuzzleFromTemplate` in [tests_python/test_puzzle_templates.py](../tests_python/test_puzzle_templates.py), `TestAddPuzzleFromTemplate` in [tests_python/test_room_builder.py](../tests_python/test_room_builder.py), `TestPuzzleTemplateHandlers` in [tests_python/test_main_escape_room.py](../tests_python/test_main_escape_room.py), and `puzzleTemplateFormValues` in [tests/escape-room.test.js](../tests/escape-room.test.js).
- [x] Attempt analytics (wrong-guess / hint-request frequency per puzzle) to help creators tune difficulty, extending the metrics spirit of §17 of [educational_rooms_feature_design.md](educational_rooms_feature_design.md). `PuzzleEngine` gained four per-puzzle counters (`_total_attempts`, `_wrong_attempts`, `_hint_requests`, `_wrong_guesses`) surfaced by `puzzle_analytics(puzzle_id)`/`list_analytics()` as `totalAttempts`, `wrongAttempts`, `solvedCount`, `distinctSolvers`, `hintsRequested`, `lockedOutUsers`, `successRate`, and a ranked `commonWrongGuesses`. **Privacy discipline (§12):** the payload is aggregated *per puzzle across all visitors*, never per named visitor — it contains no user ids at all, so it is a difficulty-tuning signal rather than surveillance of one player's struggle. **Security:** wrong guesses are recorded but a *correct* guess never is, so the analytics view can't become a backdoor that leaks the answer; the whole payload is nonetheless room-host gated in `RoomBuilderState.puzzle_analytics`/`list_puzzle_analytics`, because `commonWrongGuesses` would otherwise hand an ordinary player a free list of answers other people already ruled out. **Bounded growth (§15):** distinct tracked wrong guesses are capped per puzzle by `MAX_TRACKED_WRONG_GUESSES = 50` (already-seen guesses keep incrementing, new distinct junk is dropped) so a scripted brute-forcer can't balloon a room's memory — the same mitigation `MAX_OBJECTS_PER_TILE` applies to objects. Counting is deliberately *not* incremented for rate-limited guesses (never evaluated, so counting them would make a puzzle look far harder than it is) or for already-solved re-checks, and `reset_attempts` clears a visitor's lockout without rewriting the historical totals. `remove_puzzle` discards analytics alongside `_solved_by`/`_hints_used`/`_attempt_counts`. New `room:puzzle:analytics` handler (a `puzzleId` returns one puzzle, omitting it returns all) and a "Puzzle Analytics" builder panel rendering `formatPuzzleAnalytics` plus a coarse `puzzleDifficultySignal` band that stays `'unplayed'` below `DIFFICULTY_SAMPLE_THRESHOLD = 3` attempts rather than branding a puzzle "hard" off one unlucky guess. See `TestAttemptAnalytics` in [tests_python/test_puzzle.py](../tests_python/test_puzzle.py), `TestPuzzleAnalyticsWrapper` in [tests_python/test_room_builder.py](../tests_python/test_room_builder.py), `TestPuzzleAnalyticsHandler` in [tests_python/test_main_escape_room.py](../tests_python/test_main_escape_room.py), and `formatPuzzleAnalytics`/`puzzleDifficultySignal` in [tests/escape-room.test.js](../tests/escape-room.test.js).
- [x] Cross-room global leaderboard, contingent on the alignment decision in §16 Q4. **Resolves §16 Q4 in favour of a cross-room board.** `RoomsRegistry.global_escape_leaderboard(limit=10)` aggregates every room's `RoomBuilderState.escape_leaderboard()`, annotates each entry with its `roomId`/`roomName`, and re-sorts globally by `elapsedMs` — so the limit selects the genuinely fastest escapes rather than whichever rooms were iterated first. The registry owns the aggregation because it is the only object that can see all rooms at once; each `EscapeSessionEngine` keeps knowing only about its own room. It iterates `self.rooms` (not `self.room_builders`) so a room removed from the registry stops contributing immediately. **Documented assumption:** like all other gameplay state this is in-memory only — the board is empty after a restart until new escapes are recorded, and persisting it is blocked on the same §16 decisions as everything else. Client-side, `formatGlobalLeaderboardEntry` appends the source room name, since escape times from different rooms aren't comparable without it. New `room:escape:leaderboard:global` handler (room-scoped auth like every sibling handler — you must be in the world to read it) and a "Global Fastest Escapes" button beside the existing per-room one. See [tests_python/test_rooms_registry_global_leaderboard.py](../tests_python/test_rooms_registry_global_leaderboard.py), `TestGlobalEscapeLeaderboardHandler` in [tests_python/test_main_escape_room.py](../tests_python/test_main_escape_room.py), and `formatGlobalLeaderboardEntry` in [tests/escape-room.test.js](../tests/escape-room.test.js).
...
4. Should the leaderboard be per-room only, or eventually cross-room (a global "fastest escapes" board) once persistence exists? *(**Resolved in Phase 3: cross-room.** `RoomsRegistry.global_escape_leaderboard` and the `room:escape:leaderboard:global` handler ship alongside the existing per-room board, with each entry annotated by its source room. The "once persistence exists" caveat was **not** waited on — the board is in-memory like every other piece of gameplay state and is empty after a restart. If that proves unacceptable in practice, persisting it is part of the same §13 storage decision as everything else.)*
5. Do we want a built-in puzzle-type library (cipher, sequence, sudoku, riddle) in Phase 1, or is free-text/numeric answer matching sufficient to start, with templates deferred to Phase 3? *(**Resolved: deferred to Phase 3, now shipped.** Free-text/numeric matching was sufficient through Phases 1–2. `server/game/puzzle_templates.py` now provides `riddle`, `cipher`, `sequence`, `number_lock`, and `keyword_search`. Sudoku was **not** included: it needs a bespoke grid input widget and grid-state validation rather than the single-answer match model every other template reuses, so it stays a separate future item.)*
